  ...
  if (pokedexText && pokeEntry.species) {
    ...
    useEffect(() => {
      ...
      async function fetchData() {
        const results = [];
        for (let i = 0; i < pokedexText.varieties.length; i++) {
          const response = await fetch(pokedexText.varieties[i].pokemon.url);
              ...
              <div className="forms">
                {formData.map((item, index) => (
                  <div key={index}>
                    ...
                    <img className="officialArtWork" src={item.sprites.other.home.front_default} alt="" />
                    <div className="baseStats">
                      <table className="statsTable">
                        <tbody>
                          {item.stats.map((stat) => (
