...
function App() {
  ...
  useEffect(() => {
    if (pokedexText) {
      // pokedexText.flavor_text_entries.map(entry =>{
      //   if(entry.language.name === 'en'){
      //     console.log(entry)
      //   }
      //   return entry
      // })
      for (let i = 0; i < pokedexText.flavor_text_entries.length; i++) {
        if (pokedexText.flavor_text_entries[i].language.name === "en") {
          console.log(pokedexText.flavor_text_entries[i]);
          break;
        }
      }
    }
  ...
  return (
    <div className="App">
      <div className="column">
        <div className="row">
          <Navbar></Navbar>
          <button onClick={() => pagination(nextUrl)}>hhdff</button>
          <button onClick={() => pagination(prevUrl)}>hhdff</button>
          <SearchInput
